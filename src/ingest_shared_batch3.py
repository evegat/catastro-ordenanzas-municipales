...
urls = [
    ("perplexity_3", "https://www.perplexity.ai/search/bf27cbd0-c280-40ac-b5f1-9794483db988"),
    ("gemini_3", "https://share.gemini.google/xNLwEuigns5u")
]
...
with sync_playwright() as p:
    ...
    for name, u in urls:
        print(f"Extrayendo {name}: {u}...")
        try:
            page = browser.new_page()
            page.goto(u, timeout=30000, wait_until="domcontentloaded")
            time.sleep(4.0)
            t = page.evaluate("() => document.body.innerText")
            Path(f"data/{name}_dump.txt").write_text(t, encoding="utf-8")
            texts.append(t)
            print(f"{name} guardado ({len(t)} caracteres)")
            page.close()
...
# Extraer JSONs de todos los textos
all_records = []
for full_text in texts:
    json_matches = re.findall(r'\[\s*\{\s*"comuna".*?\}\s*\]', full_text, re.DOTALL)
...
print(f"Total registros raw extraídos: {len(all_records)}")
