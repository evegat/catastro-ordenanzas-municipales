...
import json, re, time, unicodedata, hashlib
...
print("Abriendo Perplexity con Playwright...")
with sync_playwright() as p:
    browser = p.chromium.launch(headless=True)
    page = browser.new_page()
    page.goto(url, timeout=35000, wait_until="domcontentloaded")
    time.sleep(5.0)
    # Scroll hacia abajo para renderizar respuestas largas
    page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
    time.sleep(3.0)
    t = page.evaluate("() => document.body.innerText")
    Path("data/perplexity_latest_dump.txt").write_text(t, encoding="utf-8")
    print(f"Perplexity descargado con éxito. Longitud: {len(t)} caracteres")
    browser.close()
...
text = Path("data/perplexity_latest_dump.txt").read_text(encoding="utf-8")
...
print(f"Total registros raw extraídos de Perplexity: {len(all_records)}")
...
print(f"Nuevas ordenanzas agregadas desde Perplexity: {added}")
