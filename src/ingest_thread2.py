...
# Extraer todos los fragmentos JSON del dump
json_matches = re.findall(r'\[\s*\{\s*"comuna".*?\}\s*\]', text, re.DOTALL)
print(f"Bloques JSON de lista encontrados: {len(json_matches)}")

raw_records = []
for jm in json_matches:
    try:
        obj = json.loads(jm)
        raw_records.extend(obj)
    except Exception as e:
        print(f"Error parseando bloque: {e}")

print(f"Total registros raw extraídos de Gemini Thread 2: {len(raw_records)}")
...
for r in raw_records:
    ...
    if not comuna or not target_url:
        continue
        ...
        if resp.status_code == 200:
            ...
            if header_checked and total_b > 500:
                ...
                if sha not in seen_hashes:
                    ...
                    seen_hashes.add(sha)
                    added += 1
                    print(f"[{comuna}] +1 ordenanza verificada: {doc['titulo'][:40]}")
    except Exception as e:
        continue

print(f"Proceso finalizado: +{added} nuevas ordenanzas verificadas incorporadas.")
